...
- Dependencies: ~xsel~ (for ~tmux-yank~), ~xdg-open~ (for ~tmux-open~)
...
- Dependencies: ~xmonad-contrib~, ~xmobar~, ~trayer~, ~dunst~, ~notify-send~, ~xmessage~, ~alacritty~, ~nitrogen~, ~xdotool~, ~ddate~, ~maim~
